...
export default class Drink extends React.Component {
    ...
    componentDidMount() {
        fetch("https://api.edamam.com/api/recipes/v2?type=public&app_id=c3600678&app_key=073352dcbd07fab1305f749d001d4b83&dishType=Drinks&imageSize=REGULAR")
            ...
            recipes.hits = hits;
            this.setState({drinks: recipes})
        })
    }

    nextpage = () => {
        fetch(this.state.drinks._links.next.href)
            .then(function (response) {
                return response.json();
            }).then(recipes => {
            let hits = recipes.hits.map(recipe => {
        ...
        return (
            <>
                <div className={"foodcontainer"}>
                    ...
                    {Object.keys(this.state.drinks).length > 0 ? this.state.drinks.hits.map(recipe =>
