...
export default class Maincourse extends React.Component {
    ...
    componentDidMount() {
        fetch("https://api.edamam.com/api/recipes/v2?type=public&app_id=c3600678&app_key=073352dcbd07fab1305f749d001d4b83&dishType=Main%20course&imageSize=REGULAR")
            ...
            recipes.hits = hits;
            this.setState({mains: recipes})
        })
    }

    nextpage = () => {
        fetch(this.state.mains._links.next.href)
            .then(function (response) {
                return response.json();
            }).then(recipes => {
            let hits = recipes.hits.map(recipe => {
        ...
        return (
            <>
                <div className={"foodcontainer"}>
                    ...
                    {Object.keys(this.state.mains).length > 0 ? this.state.mains.hits.map(recipe =>
                            <Link to={`/maincourse/${recipe.id}`}>
                                <div className={"foods"}>
