...
export default class Starter extends React.Component {
    ...
    nextpage = () => {
        console.log(this.state.foods._links.next.href)
        fetch(this.state.foods._links.next.href)
            .then(function (response) {
                return response.json();
            }).then(recipes => {
            let hits = recipes.hits.map(recipe => {
            ...
            this.setState({
                foods: recipes,
                selectedhrf: [...this.state.selectedhrf,this.state.foods._links.next.href],
                link:""
            })
        })
    }
